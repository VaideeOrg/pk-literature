#!/usr/bin/env bash
# Produces apps/api-feed/dist-lambda.zip — the artifact
# terraform/environments/<env>/api-feed.tf's aws_lambda_function
...
STAGING_DIR="$ROOT_DIR/.lambda-package/api-feed"
ZIP_PATH="$APP_DIR/dist-lambda.zip"

echo "==> Building workspace packages (domain-types, contracts, api-feed)"
(cd "$ROOT_DIR" && pnpm --filter @pk-literature/domain-types --filter @pk-literature/contracts --filter api-feed run build)

echo "==> Resolving a self-contained package (pnpm deploy --prod)"
rm -rf "$STAGING_DIR"
(cd "$ROOT_DIR" && pnpm --filter api-feed deploy --prod "$STAGING_DIR")
