#!/usr/bin/env bash
# Produces apps/api-identity/dist-lambda.zip — the artifact
# terraform/environments/<env>/api-identity.tf's aws_lambda_function
# references directly via `filename`/`source_code_hash`. Identical
# approach to apps/api-commerce/scripts/package-lambda.sh — see that
...
STAGING_DIR="$ROOT_DIR/.lambda-package/api-identity"
ZIP_PATH="$APP_DIR/dist-lambda.zip"

echo "==> Building workspace packages (domain-types, contracts, api-identity)"
(cd "$ROOT_DIR" && pnpm --filter @pk-literature/domain-types --filter @pk-literature/contracts --filter api-identity run build)
