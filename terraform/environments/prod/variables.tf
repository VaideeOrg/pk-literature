...
variable "directus_image_tag" {
  description = "Tag pushed by .github/workflows/build-directus-image.yml (its own directus_image_tag input, separate from the upstream directus_version build-arg). ECR tags are IMMUTABLE (terraform/bootstrap/ecr.tf), and this image bakes in our own layer on top of the pinned upstream directus/directus base (eventbridge-put-event extension, RDS CA bundle for the direct-to-RDS test, promote-staging-book, decrement-inventory-stock) — a change to that layer needs a new tag to roll out even when the upstream version hasn't moved, same as medusa_image_tag's own -N convention. The -5 suffix marks build revision 5 of upstream 12.1.1 - adds the decrement-inventory-stock operation (three types of orders / shared inventory pool work: apps/api-commerce's inventory-sync-consumer and apps/medusa's store-order creation route both call into it via a webhook Flow, see bootstrap.ts's ensureInventoryDecrementFlow). The -4 suffix's intended content was identical to what -3 was meant to be (moves both extensions from extensions/operations/<name>/ to extensions/<name>/, flat, no type subdirectory, in the Dockerfile's final COPY destinations - confirmed live that @directus/extensions' resolveFsExtensions() only recognizes extensions directly under EXTENSIONS_PATH), but -3 itself got built and pushed from main (the workflow dispatched against the wrong ref, before this Dockerfile fix had merged) - immutable ECR tags mean that mistake can't be corrected under the same tag, so -3 is permanently a stale/wrong build and must never be referenced again. Revision 2 added promote-staging-book; revision 1 only had the RDS CA bundle."
  type        = string
  default     = "12.1.1-5"
}
